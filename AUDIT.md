...
1. **P1（未着手・最重要）** — MP3出力の cdnjs 依存を断つ。上記「CSP の障害物」表 A・B の自己ホスト化。
2. **P1** — CI に `npm ci` / `npm audit --omit=dev --audit-level=high` / `npm test` を追加。
3. **P1** — 中核ロジック（AI作曲JSONの読み込み検証、localStorage 復元）に `node:test` でテストを1つ。
4. **§4 品質ゲート** — 正本 `SchoolPlan_Editor` の確定後に移植。
